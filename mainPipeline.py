...
if (len(sys.argv) != 4):
    print("python3 mainPipeline.py [gpu_id] [cfg file] [dataset_name]")
    sys.exit()

from Pix2Pose.bop_toolkit_lib import inout
from Pix2Pose.tools import bop_io, myrender
from Pix2Pose.pix2pose_model import Myrecognition as recog
from maskrcnn.maskrcnn_utils import get_model_rcnn,get_rcnn_detection
from unet.unet_utils import get_modellist_unet
...
cam_param_global, scene_cam = bop_io.get_dataset(cfg, dataset, incl_param=True,
                                                 train=False)  # TODO: check the paths here
...
# Load 21 U-net models
print("="*100),print("load 21 Unet models....."),print("="*100)
unetmodellist=get_modellist_unet("./unet/unet_config.json")
...
for m_id,model_id in enumerate(model_ids):
    ...
    recog_temp = recog.pix2pose(unetmodellist[m_id],camK= cam_K,
...
"""MaskRCNN Detection"""
# load MaskRCNN model
model = get_model_rcnn("./maskrcnn/maskrcnn_config.json")
...
for scene_id,im_id,obj_id_targets,inst_counts in target_list:
    ...
    for r_id, roi in enumerate(rois):
        ...
        if (inst_count_pred[obj_gt_no] > inst_counts[
            obj_gt_no] * cand_factor):  # If the number of detected instances is 2(can_factor) larger then the # given by the groundtruth, then we stop process this bbox
            continue
        ...
        if (score_type == 2 and detect_type == 'rcnn'):
            ...
            if (union <= 0):
                ...
                mask_iou = np.sum(np.logical_and(mask_from_detect,
                                                 mask_pred)) / union  # TODO: Why compute iou between mask_pred and mask_from_detect?
    ...
    for o_id, tf, score, roi in zip(result_objid, result_poses, result_score, result_bbox):
        ...
        cv2.putText(img_pose, '{:.3f}'.format(score), (roi[1], roi[0]), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (255, 255, 255), 2, 1)
        # print(o_id)
    cv2.imwrite(cfg["rendered_figures_dir"] + "/scene_id:{}_im_id:{}.jpg".format(scene_id, im_id),
                img_pose[:, :, ::-1])
    ...
    for result_id in sorted_id:
        ...
        result_temp = {'scene_id': scene_id, 'im_id': im_id, 'obj_id': obj_id, 'score': score, 'R': R, 't': t,
                       'time': time_spend}
