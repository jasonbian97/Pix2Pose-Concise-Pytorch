...
class pix2pose():
    def __init__(self, generator, camK, res_x, res_y, obj_param, th_ransac=3.0, th_outlier=[0.1, 0.2, 0.3],
                 th_inlier=0.1, box_size=1.5, dist_coeff=None, **kwargs):
        self.camK = camK
        self.res_x = res_x
        self.res_y = res_y
    ...
    def get_boxes(self, bbox, v_max, u_max, ct=np.array([-1]), max_w=9999):
        ...
        h = w
        v1_ori = bbox_ct_v - int(h / 2)  # expaned bbox without considering image boarder
        ...
        shift_v_max = 0
        shift_u_max = 0
        v1 = v1_ori  # (v1,v2,u1,u2) used to store the expaned bbox considering image boarder
        v2 = v2_ori
        u1 = u1_ori
        u2 = u2_ori
        if (v1_ori < 0):
        ...
        if (u2_ori > u_max):
            ...
            u2 = u_max
        vv1 = shift_v_min  # TODO： What is this???
    ...
    def est_pose(self, rgb, bbox, gt_trans=np.eye((4)), z_iter=False):
        # get bbox is used for crop
        v1_ori, v2_ori, u1_ori, u2_ori, v1, v2, u1, u2, vv1, vv2, uu1, uu2 = self.get_boxes(bbox, rgb.shape[0],
                                                                                            rgb.shape[1])
        ...
        if (base_image.shape[0] < 5 or base_image.shape[1] < 5 or image_no_mask_zero.shape[0] < 5 or
                image_no_mask_zero.shape[1] < 5):
            print("base_image.shape[0] < 5 or base_image.shape[1] < 5 or image_no_mask_zero.shape[0] < 5 or\
                image_no_mask_zero.shape[1] < 5")
        ...
        # ======================================== Stage One =========================================
        # decode, prob = self.generator_train.predict(
        #     np.expand_dims(input_img, axis=0))  # TODO: What's the meaning of 2 outputs?
        decode, prob = get_unet_pred(input_img, self.generator_train)
        decode = decode[np.newaxis,:,:,:]
        prob = prob[np.newaxis,:,:,:]
        img_pred = (decode[0] + 1) / 2
        img_pred[img_pred > 1] = 1  # TODO: Why cap?
        ...
        # set different level constraint to generate several candidate refined_images.
        for th_o in self.th_o:
            prob_mask = prob[0, :, :, 0] < th_o
            non_gray_prob = np.logical_and(non_gray, prob_mask)  # TODO: non_gray_prob [bool mask]
            ...
            bbox = bbox * np.array(
                [(v2_ori - v1_ori) / 128, (u2_ori - u1_ori) / 128, (v2_ori - v1_ori) / 128, (u2_ori - u1_ori) / 128])
            non_gray_ori = resize(non_gray_prob, (v2_ori - v1_ori, u2_ori - u1_ori), order=1, mode='constant',
                                  cval=0) > 0.9
            ...
            v1_ori_2, v2_ori_2, u1_ori_2, u2_ori_2, v1_2, v2_2, u1_2, u2_2, vv1_2, vv2_2, uu1_2, uu2_2 = self.get_boxes(
                bbox, rgb.shape[0], rgb.shape[1], ct=np.array([cy_m, cx_m]), max_w=w_stage_1)
            box_refined.append(
                [v1_ori_2, v2_ori_2, u1_ori_2, u2_ori_2, v1_2, v2_2, u1_2, u2_2, vv1_2, vv2_2, uu1_2, uu2_2])
        ...
        # ======================================== Stage Two =========================================
        # the original output image is refined, then feed the refined into the network again.
        # decode, prob = self.generator_train.predict(np.array(input_refined))  # decode,prob are 4 dims tensor
        decode=[]
        prob=[]
        for input in input_refined:
            tmp_decode, tmp_prob = get_unet_pred(np.array(input), self.generator_train)
            decode.append(tmp_decode)
            prob.append(tmp_prob)
        decode = np.array(decode) # 4 dims
        prob = np.array(prob)
        max_inlier = -1
        for cand_id in range(len(input_refined)):
            # use bbox, mask, threshold to refine the 3D map.('RGB')
            v1_ori, v2_ori, u1_ori, u2_ori, v1, v2, u1, u2, vv1, vv2, uu1, uu2 = box_refined[cand_id]
            img_prob_ori = resize(prob[cand_id, :, :, 0], (v2_ori - v1_ori, u2_ori - u1_ori), order=1, mode='constant',
                                  cval=1)
            ...
            img_pred[img_pred < 0] = 0
            img_pred_ori = resize(img_pred, (v2_ori - v1_ori, u2_ori - u1_ori), order=1, mode='constant',
                                  cval=0.5) * 255

            non_gray = resize(non_gray.astype(float), (v2_ori - v1_ori, u2_ori - u1_ori), order=1, mode='constant',
                              cval=0) > 0.9
            ...
            rgb_aug_test2[v1:v2, u1:u2] = [128, 128, 128]  # replace bacground with grey values
            rgb_aug_test2[v1:v2, u1:u2] = img_pred_ori

            rot_pred_cand, tra_pred_cand, valid_mask, n_inliers = self.pnp_ransac(rgb_aug_test2, img_prob_ori, non_gray,
                                                                                  v1, v2, u1, u2)
            # print("n_inliers:",n_inliers,rot_pred_cand,tra_pred_cand)
            n_inliers = n_inliers / n_non_gray
            if (n_inliers > max_inlier):  # track best estimated pose by # of inliers
        ...
        if (max_inlier == -1):
            ...
            return img_pred_f.astype(np.uint8), valid_mask_full, rot_pred, tra_pred, max_inlier / n_init_mask, np.array(
                [v1, v2, u1, u2], np.int)

    def pnp_ransac(self, rgb_aug_test, img_prob_ori, non_zero, v1, v2, u1, u2):
        rgb_aug_crop = rgb_aug_test[v1:v2, u1:u2]  # crop ROI
